...
├── configs.txt                     # Drive / YouTube / Gmail の ID/Secret
├── auto-conversion.json            # Drive サービスアカウント鍵
├── gphotos_auth.json               # Photos OAuth トークン
├── youtube_auth.json               # YouTube OAuth トークン
└── logs/                           # ローテーションログ出力先
```
...
| ファイル | 役割 | エクスポート形態 |
|---|---|---|
| `insta360_auto_converter.py` | エントリポイント。`main()` の `while True` ループに全体フローを書く | スクリプト (`if __name__ == '__main__': main()`) |
